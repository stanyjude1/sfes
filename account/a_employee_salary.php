...
$form_url = WEB_URL . "account/a_employee_salary.php";
$id="";
$hdnid="0";
...
if(isset($_POST['dllEmNameId'])){
	...
	mysql_close($link);
	$redirect_url = WEB_URL . 'account/a_e_salarylist.php?m=i';
	...
	mysql_query($sql,$link);
	mysql_close($link);
	$redirect_url = WEB_URL . 'account/a_e_salarylist.php?m=u&tval='.$_POST['hdnsid'].'&cval='.$_POST['hdncid'];
	header('Location: ' . $redirect_url);
	die();
}
...
if(isset($_GET['id']) && $_GET['id'] != ''){
	...
	while($row = mysql_fetch_array($result)){
		...
		$form_url = WEB_URL . "account/a_employee_salary.php?id=".$_GET['id'];
	}
...
<div class="content">
  ...
  <div class="main_content">
    <div class="main_content_left">
      ...
      <div class="my_button"><a class="btn btn_back btn-success" href="<?php echo WEB_URL; ?>account/a_e_salarylist.php">Teacher Salary List</a></div>
    </div>
    <div class="frmstyle">
      <form enctype="multipart/form-data" action="<?php echo $form_url; ?>" method="post" role="form" class="form-horizontal">
		...
		<input type="hidden" value="<?php echo $e_month; ?>" name="hdnsid"/>
        <input type="hidden" value="<?php echo $e_name; ?>" name="hdncid"/>
      </form>
    </div>
  </div>
