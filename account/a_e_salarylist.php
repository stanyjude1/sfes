...
<div class="content content_inside">
  <div class="header_text">
    ...
    <div class="top_common_bar">
		<div class="obj_left">
        <select id="ddlClass" onChange="getRollClass(this.value,'a_e_salarylist'),getRoll(this.value);" class="form-control" name="ddlClass">
      ...
      <div class="obj_left">
        <select onChange="getRollData(this.value,'a_e_salarylist');" class="form-control" name="ddlRoll" id="ddlRoll">
      ...
      <div class="obj_right" style="padding-right:10px !important;"><a class="btn btn_add_new btn-success" href="<?php echo WEB_URL; ?>account/a_employee_salary.php"><i class="fa fa-plus"></i>&nbsp;Add Employee Salary</a>&nbsp;<a class="btn btn_add_new btn-success" href="<?php echo WEB_URL; ?>account/a_account.php"><i class="fa"></i>Account</a></div>
    </div>
  </div>
	  	...
	  	<?php
			$result = '';
		  		if(isset($_GET['cval']) && $_GET['cval'] != '' && isset($_GET['tval']) && $_GET['tval'] != ''){
					...
					$result = mysql_query("Select *,u.u_name,m.m_name from tbl_add_employee_salary es inner join tbl_add_user u on es.e_name = u.u_id inner join tbl_add_month m on es.e_month = m.mid WHERE es.e_id=(SELECT max(e_id) FROM tbl_add_employee_salary)",$link);
				}
				else{
...
<script type="text/javascript">
  function deleteSalary(Id){
	...
	if(iAnswer){
		window.location = 'a_e_salarylist.php?id=' + Id;
	}
  }
  </script>
