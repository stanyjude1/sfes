...
?>
  ...
  <div>
    ...
    $result = mysql_query("Select sm.class_id,sm.subject_id,sm.student_id,sm.marks,sm.exam_id,s.s_name,s.s_id,s.s_email,s.s_contact,s.s_dob,s.s_address,c.c_name,s.s_roll_no,s.s_gender,s.s_religion,s.s_profile_name,s.s_image,p.p_father_name from tbl_student_marks sm inner join tbl_add_student s on sm.student_id = s.s_id inner join tbl_add_class c on sm.class_id = c.c_id inner join tbl_add_parent p on p.p_id = s.parent_id where s_id = '" . (int)$_SESSION['objLogin']['s_id'] . "'",$link);
		...
		$result = mysql_query("Select * from tbl_student_marks where smid = '-1' order by smid desc",$link);
	}
  ...
  <br/>
  <div>
    <div align="center" style="font-size:20px; font-weight:bold; color:black; text-align:center; margin-top:20px; margin-bottom:20px; text-decoration:underline;">FIRST TERM EXM HISTORY</div>
    <div align="left" style="width:100%; margin-bottom:30px;">
      <table class="gridtable" align="left"  style="width:100%">
        <thead>
          <tr>
        ...
        <tbody>
          <?php
		$id = '';
		$result = mysql_query("Select sm.class_id,sm.subject_id,sm.student_id,sm.marks,sm.exam_id,s.sb_name from tbl_student_marks sm inner join tbl_add_subject s on sm.subject_id = s.sb_id  where sm.class_id='".$_GET['cid']."' and sm.exam_id='".(int)TERM_1."' and sm.student_id = '" . (int)$_SESSION['objLogin']['s_id'] . "'",$link);
  ...
  <br/>
  <div>
    <div align="center" style="font-size:20px; font-weight:bold; color:black; text-align:center; margin-top:20px; margin-bottom:20px; text-decoration:underline;">SECOND TERM EXM HISTORY</div>
    <div align="left" style="width:100%; margin-bottom:30px;">
      <table class="gridtable" align="left"  style="width:100%">
        <thead>
          <tr>
  ...
  <br/>
  <div>
    <div align="center" style="font-size:20px; font-weight:bold; color:black; text-align:center; margin-top:20px; margin-bottom:20px; text-decoration:underline;">FINAL TERM EXM HISTORY</div>
    <div align="left" style="width:100%; margin-bottom:30px;">
      <table class="gridtable" align="left" style="width:100%">
        <thead>
          <tr>
