...
?>
  ...
  <div style="padding:10px;">
  <h4 class="exam_table_style">First Terminal Examination Mark Sheet</h4>
    <table class="table common_sakotable_att table-bordered table-striped dt-responsive">
      ...
      <tbody>
        <?php
		$id = '';
		$result = mysql_query("Select sm.class_id,sm.subject_id,sm.student_id,sm.marks,sm.exam_id,s.sb_name from tbl_student_marks sm inner join tbl_add_subject s on sm.subject_id = s.sb_id  where sm.student_id='".(int)$_SESSION['objLogin']['s_id']."' and sm.class_id='".(int)$_SESSION['objLogin']['s_class_id']."' and sm.exam_id='".(int)TERM_1."'",$link);
  ...
  <div style="padding:10px;">
  <h4 class="exam_table_style">Second Terminal Examination Mark Sheet</h4>
    <table class="table common_sakotable_att table-bordered table-striped dt-responsive">
      <thead>
        <tr>
          <th>Subject</th>
      ...
      <tbody>
        <?php
		$id = '';
		$result = mysql_query("Select sm.class_id,sm.subject_id,sm.student_id,sm.marks,sm.exam_id,s.sb_name from tbl_student_marks sm inner join tbl_add_subject s on sm.subject_id = s.sb_id  where sm.student_id='".(int)$_SESSION['objLogin']['s_id']."' and sm.class_id='".(int)$_SESSION['objLogin']['s_class_id']."' and sm.exam_id='".(int)TERM_2."'",$link);
				while($row = mysql_fetch_array($result)){
				 $g_name = '';
		 $g_point = '0.00';
		 $gp = getStudentGrade($row['marks'],$link);
  ...
  <div style="padding:10px;">
  <h4 class="exam_table_style">Final Examination Mark Sheet</h4>
    <table class="table common_sakotable_att table-bordered table-striped dt-responsive">
      <thead>
        <tr>
          <th>Subject</th>
      ...
      <tbody>
        <?php
		$id = '';
		$result = mysql_query("Select sm.class_id,sm.subject_id,sm.student_id,sm.marks,sm.exam_id,s.sb_name from tbl_student_marks sm inner join tbl_add_subject s on sm.subject_id = s.sb_id  where sm.student_id='".(int)$_SESSION['objLogin']['s_id']."' and sm.class_id='".(int)$_SESSION['objLogin']['s_class_id']."' and sm.exam_id='".(int)TERM_3."'",$link);
				while($row = mysql_fetch_array($result)){
				 $g_name = '';
		 $g_point = '0.00';
		 $gp = getStudentGrade($row['marks'],$link);
