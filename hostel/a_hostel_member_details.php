...
$result = mysql_query("Select *,s.s_name,s.s_email,s.s_contact,s.s_address,s.s_dob,s.s_gender,s.s_religion,s.s_image,c.c_name from tbl_add_hostel_member m inner join tbl_add_student s on m.sid = s.s_id inner join tbl_add_class c on m.cid = c.c_id where m.hmid='".$_GET['id']."'",$link);
...
?>
  ...
  <div style="padding:10px;">
	...
	<?php
    $result = mysql_query("Select *,s.s_name,s.s_email,s.s_contact,s.s_address,s.s_dob,s.s_gender,s.s_religion,s.s_image,c.c_name,h.h_name,hc.hostel_fee from tbl_add_hostel_member hm inner join tbl_add_student s on hm.sid = s.s_id inner join tbl_add_class c on hm.cid = c.c_id inner join tbl_add_hostel h on hm.hostel_name = h.h_id inner join tbl_hostel_category hc on hm.hostel_name = hc.hostel_id where hm.hmid='".$_GET['id']."'",$link);
	while($row = mysql_fetch_array($result)){?>
    ...
    <div style="padding:10px;" align="center"><a class="btn btn_add_new btn-success" href="<?php echo WEB_URL; ?>hostel/a_hostel_member_list.php?cid=<?php echo $_GET['cid']; ?>"><i class="fa fa-list"></i>&nbsp;Hostel Member List</a>&nbsp;&nbsp;<a class="btn btn_add_new btn-success" href="<?php echo WEB_URL; ?>a_dashboard.php"><i class="fa fa-home"></i>&nbsp;Dashboard</a></div>
  </div>
