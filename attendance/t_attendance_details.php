...
<?php
	...
	$id = '';
   $result = mysql_query("Select a.sid,a.cid,a.attendance,a.roll_no,a.added_date,s.s_name,s.s_id,s.s_email,s.s_contact,s.s_dob,s.s_address,c.c_name,s.s_roll_no,s.s_gender,s.s_religion,s.s_profile_name,s.s_image from tbl_student_attendance a inner join tbl_add_student s on a.sid = s.s_id inner join tbl_add_class c on a.cid = c.c_id where a.sid='".$_GET['sid']."'",$link);
...
?>
  ...
  <div>
  	<select class="form-control" onchange="getStudentAttByMonthTeacher(this.value);" id="ddlFMonth" name="ddlFMonth">
  ...
  <div style="padding:10px;" align="center"><a class="btn btn_add_new btn-success" href="<?php echo WEB_URL; ?>attendance/t_addattendance.php"><i class="fa fa-plus"></i>&nbsp;Add Attendance</a>&nbsp;&nbsp;<a class="btn btn_add_new btn-success" href="<?php echo WEB_URL; ?>attendance/t_viewattendance.php?cid=<?php echo $_GET['cid']; ?>"><i class="fa fa-list"></i>&nbsp;Back To List</a>&nbsp;&nbsp;<a class="btn btn_add_new btn-success" href="<?php echo WEB_URL; ?>t_dashboard.php"><i class="fa fa-home"></i>&nbsp;Dashboard</a></div>
</div>
