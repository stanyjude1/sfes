...
$id = '';
$result_li = mysql_query("Select * from tbl_library_member where sid='".(int)$_SESSION['objLogin']['s_id']."'",$link);
if($row_li = mysql_fetch_array($result_li)){
	$id = $row_li['lmid'];
}
$result = mysql_query("Select *,s.s_name,s.s_email,s.s_contact,s.s_address,s.s_dob,s.s_gender,s.s_religion,s.s_image,c.c_name from tbl_library_member lm inner join tbl_add_student s on lm.sid = s.s_id inner join tbl_add_class c on lm.cid = c.c_id where lm.lmid='".(int)$id."'",$link);
...
?>
  ...
  <div style="padding:10px;">
    <?php	
    $result = mysql_query("Select *,s.s_roll_no,s.s_name,s.s_email,s.s_contact,s.s_address,s.s_dob,s.s_gender,s.s_religion,s.s_image,c.c_name,p.p_father_name from tbl_library_member lm inner join tbl_add_student s on lm.sid = s.s_id inner join tbl_add_class c on lm.cid = c.c_id inner join tbl_add_parent p on p.p_id = s.parent_id where lm.lmid='".(int)$id."'",$link);
	while($row = mysql_fetch_array($result)){?>
    ...
    <div style="padding:10px;">
      <h3 class="exam_table_style">Book Issue Information :</h3>
      <table class="table common_sakotable table-bordered table-striped dt-responsive">
        ...
        <tbody>
          <?php
	   $result_att = mysql_query("Select *,m.lmid,b.book_name,b.author_name,b.subject_code from tbl_book_issue i inner join tbl_library_member m on m.lmid = i.lid inner join tbl_library_book_list b on i.bname = b.b_id where i.lid='".(int)$id."' order by i.issue_id asc",$link);
